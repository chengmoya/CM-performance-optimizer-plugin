...
class ExpressionCacheModule:
    ...
    def _should_full_rebuild(self) -> bool:
        ...
        if now - self._tracker["last_full_rebuild"] > self._full_rebuild_interval:
            logger.info("[ExprCache] 达到全量重建间隔，触发全量重建")
            return True
        ...
        if cache_size > 0:
            ...
            if ratio > self._incremental_threshold_ratio:
                logger.info(f"[ExprCache] 增量比例 {ratio:.2%} 超过阈值 {self._incremental_threshold_ratio:.2%}，触发全量重建")
                return True
        ...
        try:
            ...
            with self.buffer_lock:
                if self.buffer_a is None or self._tracker["last_max_id"] == 0:
                    logger.info("[ExprCache] 首次加载，执行全量加载...")
                    async with self.load_lock:
                        self.loading = False
                    await self._load_to_buffer_b()
                    return

            logger.info("[ExprCache] 开始增量加载表达式缓存...")
            t0 = time.time()
